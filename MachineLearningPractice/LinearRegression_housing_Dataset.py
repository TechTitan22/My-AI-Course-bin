...
df.plot.scatter(x='median_income', y='median_house_value', title='Scatter Plot of median_income and median_house_value percentages');
plt.show()

'''
`df.corr()` uses the entire DataFrame because correlation is a **statistical operation between all columns**, and pandas has **no concept of X (features) or y (target)**.
It simply computes relationships for **every column present**, without making ML assumptions.
'''
# that's why we put inside the brackets (numeric_only=True)  -> it consider the just numeric values an dignore all the strings 
...
print(df['median_income'].values) 
print(df['median_house_value'].values.shape)
